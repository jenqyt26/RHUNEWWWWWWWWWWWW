...
<div class="max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800">Edit Barangay</h2>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('barangays.update', $barangay) }}" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label for="barangay_name" class="block text-sm font-medium text-gray-700 mb-2">Barangay Name</label>
                <input type="text" id="barangay_name" name="barangay_name" value="{{ $barangay->barangay_name }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4332] focus:border-transparent">
                @error('barangay_name')
