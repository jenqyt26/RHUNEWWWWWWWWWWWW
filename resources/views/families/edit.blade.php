...
<div class="max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800">Edit Family</h2>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('families.update', $family) }}" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label for="family_name" class="block text-sm font-medium text-gray-700 mb-2">Family Name</label>
                <input type="text" id="family_name" name="family_name" value="{{ $family->family_name }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4332] focus:border-transparent">
                @error('family_name')
            ...
            <div>
                ...
                <select id="barangay_id" name="barangay_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4332] focus:border-transparent">
                    ...
                    @foreach($barangays as $barangay)
                        <option value="{{ $barangay->id }}" {{ $family->barangay_id == $barangay->id ? 'selected' : '' }}>
