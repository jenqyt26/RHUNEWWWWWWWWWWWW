    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('patients.update', $patient) }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" id="patient_name" name="patient_name" value="{{ $patient->patient_name }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4332] focus:border-transparent">
                @error('patient_name')
            ...
            <div>
                ...
                <select id="family_id" name="family_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4332] focus:border-transparent">
                    ...
                    @foreach($families as $family)
                        <option value="{{ $family->id }}" {{ $patient->family_id == $family->id ? 'selected' : '' }}>
                            {{ $family->family_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" id="birthdate" name="birthdate" value="{{ $patient->birthdate }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4332] focus:border-transparent">
                @error('birthdate')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" id="contact_number" name="contact_number" value="{{ $patient->contact_number }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a4332] focus:border-transparent">
